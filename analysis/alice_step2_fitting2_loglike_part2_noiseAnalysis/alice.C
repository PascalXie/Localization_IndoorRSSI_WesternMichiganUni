...
void RaisdualTest();
void Analysis(double P0, double gamma, double dist_shift, double min, double max);
...
int main()
{
	...
	string filename2 = "../../alice_step2_fitting2_loglike_part1/build/data_step2_logLike.txt";
	...
	read2>>name>>gamma;
	read2>>name>>dist_0;
	read2>>name>>dist_shift;
	cout<<"Fitting Results : P0 "<<P0<<", Gamma "<<gamma<<", dist_shift "<<dist_shift<<endl;
	...
	for(int i=0;i<binSize;i++)
	{
		...
		Analysis(P0, gamma, dist_shift, current_min, current_max);
	}
...
void Analysis(double P0, double gamma, double dist_shift, double min, double max)
{
	...
	double mean_rssi = P0 - 10.*log10(mean_dist-dist_shift/1.);
