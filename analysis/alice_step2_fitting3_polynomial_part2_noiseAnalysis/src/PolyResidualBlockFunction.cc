...
bool PolyResidualBlockFunction::ResidualFunction(vector<double> variables, vector<double> &residuals)
{
	...
	// observations_[0] : P_mean, anchor observed power strength, mean
	// observations_[1] : pdf, the value of probability density function
	// observations_[2] : P_ob, anchor observed power strength

	// varialbles[0] : sigma, for the Gaussian distribution

	// residual = (-1*ln(sqrt(2PI)*sigma) - (P_ob-P_mean)^2/(2*sigma^2) - ln pdf )^2

	double P_mean	= observations_[0];
	double pdf		= observations_[1];
	double P_ob		= observations_[2];

	double sigma   = variables[0];

	double residual_0 = 0;
	residual_0 = -log(sqrt(2.*PI)*sigma) - (P_ob-P_mean)*(P_ob-P_mean)/(2.*sigma*sigma) - log(pdf);
