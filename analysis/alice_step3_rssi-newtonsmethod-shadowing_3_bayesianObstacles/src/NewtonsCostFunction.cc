	...
	for(int i=0;i<ResidualBlockFunctions_.size();i++)
	{
		...
		*/
		...
		for(int i=0;i<SizeResiduals_;i++)
		{
			...
			//
			for(int j=0;j<nosignal_ancxs_.size();j++)
			{
				int noSignalAnchorID = j;
				double dist2 = 0;
				dist2 = dist2 + (variables[0]-nosignal_ancxs_[noSignalAnchorID])*(variables[0]-nosignal_ancxs_[noSignalAnchorID]);
				dist2 = dist2 + (variables[1]-nosignal_ancys_[noSignalAnchorID])*(variables[1]-nosignal_ancys_[noSignalAnchorID]);
				
				double pdfGaussian = nosignal_weight_ * 1./(sqrt(2.*PI)*nosignal_sigma_) * exp(-1.*dist2/(2.*nosignal_sigma_*nosignal_sigma_));
				single_costFunctionValue_current += pdfGaussian;
			}

			//
			// obstacles for bayesian , rooms, walls
			//
			for(int j=0;j<obstacle_obsxs_.size();j++)
			{
				int obstacleID = j;
				double dist2 = 0;
				dist2 = dist2 + (variables[0]-obstacle_obsxs_[obstacleID])*(variables[0]-obstacle_obsxs_[obstacleID]);
				dist2 = dist2 + (variables[1]-obstacle_obsys_[obstacleID])*(variables[1]-obstacle_obsys_[obstacleID]);

				double pdfGaussian = obstacle_weight_ * 1./(sqrt(2.*PI)*obstacle_sigma_) * exp(-1.*dist2/(2.*obstacle_sigma_*obstacle_sigma_));
				single_costFunctionValue_current += pdfGaussian;
			}
...
//-------------------------
// Public:  no signal anchors for bayesian
//-------------------------
void NewtonsCostFunction::SetBayesianAnchors(vector<double> ancxs, vector<double> ancys, double sigma, double weight)
{
	nosignal_ancxs_ = ancxs;
	nosignal_ancys_ = ancys;
	nosignal_sigma_ = sigma;
	nosignal_weight_ = weight;
}

//-------------------------
// Public: obstacles for bayesian , rooms, walls
//-------------------------
void NewtonsCostFunction::SetBayesianObstacles(vector<double> obsxs, vector<double> obsys, double sigma, double weight)
{
	obstacle_obsxs_ = obsxs;
	obstacle_obsys_ = obsys;
	obstacle_sigma_ = sigma;
