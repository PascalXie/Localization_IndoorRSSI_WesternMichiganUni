...
int main()
{
	...
	//
	double UserReferencedLength = 7.;
	manager->SetUserReferencedLength(UserReferencedLength);

	// 
	double UserReferencedEpsilon = 2e2;
	...
	double dist_shifted = -2;
	cout<<"Fitting results : P0 "<<results[0]<<", gamma "<<results[1]<<", dist_shifted "<<dist_shifted<<endl;

	ofstream write("data_step2_logLike.txt");
	write<<"P0 "<<results[0]<<endl;
	write<<"Gamma "<<results[1]<<endl;
	write<<"dist0 "<<1<<endl;
	write<<"dist_shift "<<dist_shifted<<endl;
	write.close();
